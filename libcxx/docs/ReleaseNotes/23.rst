...
- P4037R1: Supporting ``signed char`` and ``unsigned char`` in random number generation (`Github <https://llvm.org/PR189609>`__)
...
- Inserting a distribution whose ``IntType`` is ``signed char`` or ``unsigned char`` into a stream now writes the
  distribution's parameters as numbers instead of as characters, and extraction reads them back as numbers. These types
  were previously supported as an extension (spelled ``std::int8_t``/``std::uint8_t``) whose textual representation
  didn't round-trip, so representations written by older versions of libc++ can't be read back.
